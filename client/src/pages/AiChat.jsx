...
const AnalysisPanel = ({ analysis }) => {
    const maxScore = Math.max(...analysis.topCategories.map(c => c.score), 0.001);
        ...
        <div className="ai-analysis-panel">
            <div className="panel-header">
                ...
                <span className={`confidence-badge ${analysis.confidence}`}>
                    {analysis.confidence === 'high' ? '🎯 Cao' :
                        analysis.confidence === 'medium' ? '🤔 Trung bình' : '❓ Thấp'}
                </span>
            </div>
                    ...
                    {analysis.topCategories.map((cat, i) => (
                        <div className="score-row" key={cat.category}>
                            <div className="score-label">
                                <span>{i === 0 ? '🏆' : i === 1 ? '🥈' : '🥉'} {cat.category}</span>
                    ...
                    <>
                        ...
                        <div className="token-chips">
                            {analysis.inputTokens.slice(0, 12).map((token, i) => (
                                <span className="token-chip" key={i}>{token}</span>
                            ))}
                            {analysis.inputTokens.length > 12 && (
                                <span className="token-chip">+{analysis.inputTokens.length - 12}</span>
                            )}
                        </div>
                    </>
                )}
...
const BookResults = ({ books, category }) => {
    ...
    return (
        <div className="ai-books-section">
            ...
            <div className="ai-books-grid">
                {books.map(book => (
                    <Link to={`/book/${book._id}`} className="ai-book-card" key={book._id}>
                        ...
                        <div className="ai-book-info">
                            ...
                            <div className="ai-book-footer">
                                <span className="ai-book-price">{formatPrice(book.price)}</span>
                                <span className="ai-book-rating">⭐ {book.rating?.toFixed(1) || '–'}</span>
                            </div>
                        </div>
                    </Link>
                ))}
                ...
                <div className="chat-window">
                    ...
                    <div className="chat-input-bar">
                        <textarea
                            ...
                            placeholder="Mô tả sở thích đọc sách của bạn... (Enter để gửi)"
                            value={input}
